...
export default function Dashboard() {
  ...
  useEffect(() => {
    authFetch("https://sammy-back.onrender.com/api/dashboard")
      .then((res) => res.json())
      .then((data) => {
        setStats(data);
        setLoading(false);
  ...
  return (
    <div className="space-y-10 bg-[#FAF3E3] min-h-screen py-8 px-2 md:px-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1
            className={`text-4xl font-bold text-[#7A5C3D] mb-1 ${dancing.className}`}
          >
            Dashboard
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div className="bg-[#FFF8F0] rounded-2xl shadow-lg p-8 border-2 border-[#F6E3B4] flex flex-col items-center">
          ...
          <div className="text-3xl font-bold text-[#7A5C3D]">
        ...
        <div className="bg-[#FFF8F0] rounded-2xl shadow-lg p-8 border-2 border-[#F6E3B4] flex flex-col items-center">
          <span className="text-5xl mb-3">��</span>
          <div className="text-3xl font-bold text-[#7A5C3D]">
            {formatCurrency(Number(stats.valorTotalEstoque || 0))}
          </div>
          <div className="text-[#BFA181] text-lg">Valor total em estoque</div>
        </div>
        <div className="bg-[#FFF8F0] rounded-2xl shadow-lg p-8 border-2 border-[#F6E3B4] flex flex-col items-center">
          <span className="text-5xl mb-3">⚠️</span>
          <div className="text-3xl font-bold text-[#C2A85D]">
            {stats.materiaisBaixoEstoque}
          </div>
          <div className="text-[#BFA181] text-lg">
            Materiais com estoque baixo
          </div>
        </div>
        <div className="bg-[#FFF8F0] rounded-2xl shadow-lg p-8 border-2 border-[#F6E3B4]">
          <div className="font-bold text-[#7A5C3D] mb-2 text-lg">
            Receitas Populares
          </div>
          {Array.isArray(stats.receitasPopulares) &&
          stats.receitasPopulares.length > 0 ? (
        ...
        <div className="bg-[#FFF8F0] rounded-2xl shadow-lg p-8 border-2 border-[#F6E3B4] col-span-1 md:col-span-2 lg:col-span-3">
          ...
          {Array.isArray(stats.pedidosRecentes) &&
          stats.pedidosRecentes.length > 0 ? (
            <ul className="text-[#7A5C3D] text-base space-y-1">
              {stats.pedidosRecentes.map((p: any) => (
                <li key={p.id}>
                  <span className="font-semibold">{p.nomeCliente}</span> —{" "}
                  {new Date(p.dataPedido).toLocaleString()} — {p.status}
                </li>
              ))}
            </ul>
          ) : (
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-[#F6E3B4]">
        <h3
          className={`text-2xl font-semibold text-[#7A5C3D] mb-4 ${dancing.className}`}
        >
          Distribuição do Valor de Estoque por Categoria
        </h3>
        {Array.isArray(stats.custosPorCategoria) &&
        stats.custosPorCategoria.length > 0 ? (
          <Pie
            data={{
            ...
            options={{
              plugins: {
                legend: {
                  ...
                  labels: {
                    color: "#7A5C3D",
                    font: { family: "Dancing Script, cursive", size: 16 },
                  },
                },
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-[#F6E3B4]">
        ...
        <div className="space-y-4">
          <div className="flex items-center p-3 bg-[#F7C5CC]/40 rounded-lg">
            ...
            <div>
              <p className="text-base font-medium text-[#7A5C3D]">
                Receita "Bolo de Chocolate" criada
              </p>
          ...
          <div className="flex items-center p-3 bg-[#F6E3B4]/40 rounded-lg">
            ...
            <div>
              <p className="text-base font-medium text-[#7A5C3D]">
                Material "Farinha de Trigo" atualizado
              </p>
      ...
      <div className="bg-gradient-to-r from-[#F7C5CC] to-[#F6E3B4] p-8 rounded-2xl text-[#7A5C3D] shadow-lg border-2 border-[#F6E3B4]">
        <h3 className={`text-2xl font-semibold mb-3 ${dancing.className}`}>
          💡 Dicas para Otimizar
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-base">
          <div className="flex items-start">
            ...
            <p>
              Monitore regularmente os custos dos materiais para manter a
              lucratividade
            </p>
          </div>
          <div className="flex items-start">
